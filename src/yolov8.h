...
public:
	...
	YoloV8(const std::string &onnxModelPath,
		   const float probabilityThreshold = 0.25f,
		   const float nmsThreshold = 0.65f,
		   const int topK = 100);

	// Detect the objects in the image
	std::vector <Object> detectObjects(const cv::Mat &img_BGR);
...
private:
	...
	std::vector <std::vector<cv::cuda::GpuMat>> preprocess(const cv::Mat &img_BGR);

	// Post-processing the output
	std::vector <Object> postprocess(std::vector<float> &feature_vector);
	...
	std::unique_ptr <Engine> trt_engine = nullptr;
	...
	// cv::Mat dimensions for the image
	float m_ratio;
	float m_img_width;
	float m_img_height;
